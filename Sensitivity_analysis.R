...
#This allows to process the sensitivity analysis results from CaveCalc.
...
#LG dataset
LG_sens <- read.mat("Data/CAN_15092020_sensitivity/CAN_15092020_LGM_sens.mat")
CaveCalc_out_LG_sens <- CaveCalc_extr(LG_sens)
CaveCalc_out_LG_sens <- as.data.frame(CaveCalc_out_LG_sens)
CaveCalc_out_LG_sens <- CaveCalc_out_LG_sens[CaveCalc_out_LG_sens$d13C!=-999, ]

#EH dataset
EH_sens <- read.mat("Data/CAN_15092020_sensitivity/CAN_15092020_EH_sens.mat")
CaveCalc_out_EH_sens <- CaveCalc_extr(EH_sens)
CaveCalc_out_EH_sens <- as.data.frame(CaveCalc_out_EH_sens)
CaveCalc_out_EH_sens <- CaveCalc_out_EH_sens[CaveCalc_out_EH_sens$d13C!=-999, ]

#Extract the model parameters for each simulation
parameters_EH <- apply(CaveCalc_out_EH_sens[1:5], 2, unique)
parameters_EH
parameters_LG <- apply(CaveCalc_out_LG_sens[1:5], 2, unique)
parameters_LG
...
#CaveCalc_out_EH <- tibble::add_column(CaveCalc_out_EH_sens, ts=NA) 
#CaveCalc_out_LG <- tibble::add_column(CaveCalc_out_LG_sens, ts=NA)

#Here, we use a different function to match model and data, which selects the 5% model results that best
#match the data. This is to avoid having too many datapoints without matching results (as fixed d13C results in
#less flexibility of the model).
proxy.DCP.soln.EH  <- eucl_proxy(model = CaveCalc_out_EH_sens, model.col = "DCP", proxy = stal_data[1:2,], proxy.col = "DCP", proxy.age = 2)
proxy.DCP.soln.LG  <- eucl_proxy(model = CaveCalc_out_LG_sens, model.col = "DCP", proxy = stal_data[3:7,], proxy.col = "DCP", proxy.age = 2)
proxy.DCP.soln     <- rbind(proxy.DCP.soln.EH, proxy.DCP.soln.LG)
proxy.Ca.soln.EH   <- eucl_proxy(model = CaveCalc_out_EH_sens, model.col = "d44Ca", proxy = stal_data[1:2,], proxy.col = "d44Ca", proxy.age = 2)
proxy.Ca.soln.LG   <- eucl_proxy(model = CaveCalc_out_LG_sens, model.col = "d44Ca", proxy = stal_data[3:7,], proxy.col = "d44Ca", proxy.age = 2)
proxy.Ca.soln      <- rbind(proxy.Ca.soln.EH, proxy.Ca.soln.LG)
proxy.d13C.soln.EH <- eucl_proxy(model = CaveCalc_out_EH_sens, model.col = "d13C", proxy = stal_data[1:2,], proxy.col = "d13C", proxy.age = 2)
proxy.d13C.soln.LG <- eucl_proxy(model = CaveCalc_out_LG_sens, model.col = "d13C", proxy = stal_data[3:7,], proxy.col = "d13C", proxy.age = 2)
...
colnames(proxy.DCP.soln.EH)[1] <- "ts"
colnames(proxy.DCP.soln.LG)[1] <- "ts"
colnames(proxy.DCP.soln)[1] <- "ts"
colnames(proxy.Ca.soln.EH)[1] <- "ts"
colnames(proxy.Ca.soln.LG)[1] <- "ts"
colnames(proxy.Ca.soln)[1] <- "ts"
colnames(proxy.d13C.soln.EH)[1] <- "ts"
colnames(proxy.d13C.soln.LG)[1] <- "ts"
colnames(proxy.d13C.soln)[1] <- "ts"
...
#Solution d13C
proxy.fin.soln1.EH <- constr_soln2(soln = proxy.d13C.soln.EH, soln.col = "DCP", proxy = stal_data[1:2,], proxy.col = "DCP", proxy.age = 2)
proxy.fin.soln1.LG <- constr_soln2(soln = proxy.d13C.soln.LG, soln.col = "DCP", proxy = stal_data[3:7,], proxy.col = "DCP", proxy.age = 2)
...
proxy.fin.soln2.EH <- constr_soln2(soln = proxy.fin.soln1.EH, soln.col = "d44Ca", proxy = stal_data[1:2,], proxy.col = "d44Ca", proxy.age = 2)
proxy.fin.soln2.LG <- constr_soln2(soln = proxy.fin.soln1.LG, soln.col = "d44Ca", proxy = stal_data[3:7,], proxy.col = "d44Ca", proxy.age = 2)
...
proxy.fin.soln3.EH <- constr_soln2(soln = proxy.d13C.soln.EH, soln.col = "d44Ca", proxy = stal_data[1:2,], proxy.col = "d44Ca", proxy.age = 2)
proxy.fin.soln3.LG <- constr_soln2(soln = proxy.d13C.soln.LG, soln.col = "d44Ca", proxy = stal_data[3:7,], proxy.col = "d44Ca", proxy.age = 2)
...
proxy.fin.soln4.EH <- constr_soln2(soln = proxy.fin.soln3.EH, soln.col = "DCP", proxy = stal_data[1:2,], proxy.col = "DCP", proxy.age = 2)
proxy.fin.soln4.LG <- constr_soln2(soln = proxy.fin.soln3.LG, soln.col = "DCP", proxy = stal_data[3:7,], proxy.col = "DCP", proxy.age = 2)
...
proxy.fin.soln5.EH <- constr_soln2(soln = proxy.DCP.soln.EH, soln.col = "d44Ca", proxy = stal_data[1:2,], proxy.col = "d44Ca", proxy.age = 2) 
proxy.fin.soln5.LG <- constr_soln2(soln = proxy.DCP.soln.LG, soln.col = "d44Ca", proxy = stal_data[3:7,], proxy.col = "d44Ca", proxy.age = 2)
...
proxy.fin.soln6.EH <- constr_soln2(soln = proxy.fin.soln5.EH, soln.col = "d13C", proxy = stal_data[1:2,], proxy.col = "d13C", proxy.age = 2) 
proxy.fin.soln6.LG <- constr_soln2(soln = proxy.fin.soln5.LG, soln.col = "d13C", proxy = stal_data[3:7,], proxy.col = "d13C", proxy.age = 2)
...
proxy.fin.soln7.EH <- constr_soln2(soln = proxy.DCP.soln.EH, soln.col = "d13C", proxy = stal_data[1:2,], proxy.col = "d13C", proxy.age = 2) 
proxy.fin.soln7.LG <- constr_soln2(soln = proxy.DCP.soln.LG, soln.col = "d13C", proxy = stal_data[3:7,], proxy.col = "d13C", proxy.age = 2)
...
proxy.fin.soln8.EH <- constr_soln2(soln = proxy.fin.soln7.EH, soln.col = "d44Ca", proxy = stal_data[1:2,], proxy.col = "d44Ca", proxy.age = 2) 
proxy.fin.soln8.LG <- constr_soln2(soln = proxy.fin.soln7.LG, soln.col = "d44Ca", proxy = stal_data[3:7,], proxy.col = "d44Ca", proxy.age = 2)
...
proxy.fin.soln9.EH <- constr_soln2(soln = proxy.Ca.soln.EH, soln.col = "DCP", proxy = stal_data[1:2,], proxy.col = "DCP", proxy.age = 2) 
proxy.fin.soln9.LG <- constr_soln2(soln = proxy.Ca.soln.LG, soln.col = "DCP", proxy = stal_data[3:7,], proxy.col = "DCP", proxy.age = 2)
...
proxy.fin.soln10.EH <- constr_soln2(soln = proxy.fin.soln9.EH, soln.col = "d13C", proxy = stal_data[1:2,], proxy.col = "d13C", proxy.age = 2) 
proxy.fin.soln10.LG <- constr_soln2(soln = proxy.fin.soln9.LG, soln.col = "d13C", proxy = stal_data[3:7,], proxy.col = "d13C", proxy.age = 2)
...
proxy.fin.soln11.EH <- constr_soln2(soln = proxy.Ca.soln.EH, soln.col = "d13C", proxy = stal_data[1:2,], proxy.col = "d13C", proxy.age = 2) 
proxy.fin.soln11.LG <- constr_soln2(soln = proxy.Ca.soln.LG, soln.col = "d13C", proxy = stal_data[3:7,], proxy.col = "d13C", proxy.age = 2)
...
proxy.fin.soln12.EH <- constr_soln2(soln = proxy.fin.soln11.EH, soln.col = "DCP", proxy = stal_data[1:2,], proxy.col = "DCP", proxy.age = 2) 
proxy.fin.soln12.LG <- constr_soln2(soln = proxy.fin.soln11.LG, soln.col = "DCP", proxy = stal_data[3:7,], proxy.col = "DCP", proxy.age = 2)
...
proxy.fin.soln <- rbind(proxy.fin.soln2, proxy.fin.soln4, proxy.fin.soln6, proxy.fin.soln8, proxy.fin.soln10, 
                        proxy.fin.soln12)
proxy.fin.soln <- proxy.fin.soln[ -c(1,2) ]
...
#d44Ca over time. Each dot represents a solution and the colour-coding shows the soil pCO2, cave pCO2, and gas volume
#of that solution.
Ca.soilCO2 <- 
...
#d13C over time. Each dot represents a solution and the colour-coding shows the soil pCO2, cave pCO2, and gas volume
#of that solution.
d13C.soilCO2 <- 
...
#Save data
write.csv(proxy.fin.soln,'solution_sens.csv')
write.csv(median.fin.soln, 'median_sens.csv')
write.csv(lower.quantile.fin.soln, 'lowerQ_sens.csv')
write.csv(upper.quantile.fin.soln, 'upperQ_sens.csv')
write.csv(res.d13C, 'resd13C_sens.csv')
